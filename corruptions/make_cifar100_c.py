...
# test_data = dset.CIFAR10('~/joseph/download/', train=False)
...
for method_name in d.keys():
    ...
    for severity in range(1,6):
        cifar_c, labels = [], []
        save_path = './CIFAR-100-C/train/' + str(severity)
        ...
        np.save(os.path.join(save_path, d[method_name].__name__ + '.npy'),
                np.array(cifar_c).astype(np.uint8))
...
train_data = dset.CIFAR100('./', train=False)
...
for method_name in d.keys():
    ...
    for severity in range(1,6):
        cifar_c, labels = [], []
        save_path = './CIFAR-100-C/test/' + str(severity)
